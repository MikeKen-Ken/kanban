...
- Agent 在会话内跑与本卡改动直接相关的定向检查，通过后才能声明完成；不要把测试命令交给 Worker 代跑，也不要把上一条的全量 `flutter analyze` / 全量测试当作默认收尾。卡住、超时不得当成通过；未做的 UI 验证须标明未执行。`ready_to_submit` 时若测试命令仍按其 `executionTime` 未结束（含 SDK 提前 completed），或最后一次测试失败，必须拒绝。
