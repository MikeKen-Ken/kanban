...
- **自动同步默认关闭**：在设置页开启后，应用启动、返回前台、本地改动后短防抖，以及应用运行时每 10 分钟执行三路合并。应用关闭后没有后台同步任务。
...
- 保存 WebDAV 配置：开启自动同步时立即尝试合并并启动应用内定时器；关闭时停止定时器
- 配置项：`WebDavConfig.enabled`、`autoSync`；`autoPull` 保留兼容字段
